...
namespace Bank
{
    public partial class Transfers : Form
    {
        public Transfers()
        {
            ...
            if (CurrencyAccountForm.boxValue == "RUB")
            {
                iHaveComboBox.SelectedItem = CurrencyAccountForm.boxValue;
            }
            if (CurrencyAccountForm.boxValue == "USD")
            {
                iHaveComboBox.SelectedItem = CurrencyAccountForm.boxValue;
            }
            if (CurrencyAccountForm.boxValue == "TNG")
            {
                iHaveComboBox.SelectedItem = CurrencyAccountForm.boxValue;
            }
                ...
                if (textBox2.Text != "0")
                {
                    if (iHaveComboBox.SelectedItem == "RUB")
                    {
                        if (Objects.user.rub.Balance - value > 0)
                        {
                            if (iWillHaveComboBox.SelectedItem == "TNG")
                            {
                                Operation add = new Operation(Convert.ToDecimal(textBox1.Text), false, "Другое", "Переводы", DateTime.Now);
                                Objects.user.rub.Balance -= Convert.ToDecimal(textBox1.Text);
                                Objects.user.rub.Add(add, true);
                                isTransferSuccsec = true;


                                add = new Operation(value, true, "Другое", "Пополнение", DateTime.Now);
